%macro varListPattern(dsn, pattern, varType,runType);
  ...
  die mit einem bestimmten Muster �bereinstimmen.

  Platzhalter f�r Muster
  "*", 0 - n Zeichen
  "?", 1 Charakter
...
%syntax:
  ...
  %if %sysevalf( %superq(varType) = , boolean ) = 1 %then
    %let varType =CN;
  %else %let varType=%qupcase(%superq(varType));

  %if %superq(varType) =A %then
    %let varType =CN;

  /* If incorrect variable type specification, throw ERROR */
  %if not(%index(ACN,%superq(varType))) %then %do;
    %let MSGTYPE=ERROR;
  ...
  %if %eval( %sysfunc( exist(&dsn, DATA) ) or %sysfunc( exist(&dsn, VIEW) ) ) = 0 %then %do;
    %let MSGTYPE=ERROR;
  ...
  %if %sysevalf( %superq(Varlist) = , boolean ) = 1 %then %do;
  ...
  %else %if %superq(runType)=D %then %do;
    options ls=90;
    %put NOTE: (&sysmacroname) &=dsn &=varType &=pattern &=nvars &=prxPattern;
    %put;
    %put NOTE- &=varlist;
    %put;
    options ls=&ls;
